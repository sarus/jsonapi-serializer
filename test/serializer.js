...
describe.only('JsonApiSerializer New Usage Tests', function() {
  describe('Serializer', function(){
    it('should be reusable', function(done){
      ...
      var json = serializer.serialize(dataSet);

      expect(json.data[0].id).equal('54735750e16638ba1eee59cb');

      json = serializer.serialize(dataSet);
      expect(json.data[0].id).equal('54735750e16638ba1eee59cb');
  ...
  describe('Options', function () {
    ...
    describe('pluralizeType', function () {
      it('should allow type to not be pluralized', function (done) {
        ...
        var serializer = new JsonApiSerializer('user', {
          attributes: ['firstName', 'lastName'],
          pluralizeType: false
        });
        ...
        serializer = new JsonApiSerializer('user', {
          attributes: ['firstName', 'lastName'],
          pluralizeType: true
        });
    ...
    describe('keyForAttribute case strings', function () {
      ...
      it('should default the key case to dash-case', function (done) {
        var serializerNoCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
        });

        var jsonNoCase = serializerNoCase.serialize(dataSet);

        var serializerInvalidCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'invalid case name'
        });

        var jsonInvalidCase = serializerInvalidCase.serialize(dataSet);
      ...
      it('should update the key case to dash-case', function (done) {
        var serializerDashCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'dash-case'
        });
        var jsonDashCase = serializerDashCase.serialize(dataSet);

        var serializerLispCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'lisp-case'
        });
        var jsonLispCase = serializerLispCase.serialize(dataSet);

        var serializerSpinalCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'spinal-case'
        });
        var jsonSpinalCase = serializerSpinalCase.serialize(dataSet);

        var serializerKebabCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'kebab-case'
        });
        var jsonKababCase = serializerKebabCase.serialize(dataSet);
      ...
      it('should update the key case to underscore_case', function (done) {
        var serializerUnderscoreCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'underscore_case'
        });
        var jsonUnderscoreCase = serializerUnderscoreCase.serialize(dataSet);

        var serializerSnakeCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'snake_case'
        });
        var jsonSnakeCase = serializerSnakeCase.serialize(dataSet);
      ...
      it('should update the key case to CamelCase', function (done) {
        ...
        var serializerCamelCase = new JsonApiSerializer('user', {
          attributes: ['firstName'],
          keyForAttribute: 'CamelCase'
        });
        var jsonCamelCase = serializerCamelCase.serialize(dataSet);
  ...
  describe('JSON API Serializer', function () {
    ...
    describe('Nested document', function () {
      ...
      it('should not contains attributes key', function (done) {
        ...
        var serializer = new JsonApiSerializer('tags', {
          ref: true,
          included: false,
          topLevelLinks: {
